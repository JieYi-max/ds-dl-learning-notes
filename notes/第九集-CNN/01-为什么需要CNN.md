...
**一张图片不是只看一次，而是被 1000 个神经元同时看了一遍。**
...
每个神经元用自己的一套权重去分析整张图，然后汇总意见 → 综合判断图片内容。

**到了深层网络，每层都在重复这个"看"的过程，但看的对象越来越抽象：**

```
第 1 层：看原始像素 → 找出边缘、颜色
第 2 层：看边缘的组合 → 找出形状（圆形、方形）
第 3 层：看形状的组合 → 找出物体部件（眼睛、耳朵）
第 4 层：看部件的组合 → 认出"这是一只猫"
```

**跟之前学的全连接层对应：**

```python
nn.Linear(784, 256)    # 256 个神经元，每个都看了一遍全部 784 个像素
nn.Linear(256, 64)     # 64 个神经元，每个都看了一遍上一层的 256 个特征
nn.Linear(64, 10)      # 10 个神经元，最终输出 10 个类别的分数
```
